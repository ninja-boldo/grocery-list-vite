/**
 * EXAMPLE: Updated Container component using new API
 * 
 * This is an example showing how to migrate from the old API to the new one.
 * The old Container.tsx can still use legacy endpoints, but this shows
 * the recommended approach for better performance and maintainability.
 */

import { useEffect, useRef, useState } from "react";
import { incrementItem, decrementItem } from "../utils/api-client";
...
const ContainerExample = ({
  ...
  // NEW APPROACH: Use the centralized API client
  const handleIncreaseClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      await incrementItem(text, false);
      
      // Option 2: Use the generic updateItem if you need more control
      // await updateItem({ itemName: text, count: 1, isWishList: false });
  ...
  const handleDecreaseClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      await decrementItem(text, false);
    ...
    <div className="mb-4 px-4">
      <div
        ...
        className={`
          ...
          border border-cyan-500/30 rounded-2xl shadow-lg shadow-cyan-500/10
          p-4 transition-all duration-300 ease-out
          hover:shadow-xl hover:shadow-cyan-400/20
          hover:border-cyan-400/50 hover:from-slate-800/95 hover:to-slate-700/95
          hover:scale-[1.02] hover:-translate-y-1 w-full backdrop-blur-md
          ...
          <div className="flex items-center gap-2 ml-4">
            ...
            <button
              ...
              className={`
                ...
                text-white font-bold text-lg rounded-xl shadow-lg shadow-rose-500/25
                hover:from-rose-400/95 hover:to-pink-500/95
                hover:shadow-xl hover:shadow-rose-400/40 hover:scale-105
                active:scale-95 disabled:scale-100 disabled:animate-pulse
                transition-all duration-200 flex justify-center items-center
      ...
      <div className={`transition-all duration-500 ease-out ${open && perish_dates_arr[0] !== "none" ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`} style={{ overflow: 'hidden' }}>
