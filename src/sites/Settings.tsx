...
const readPlannerSettings = (): PlannerSettings => {
  try {
    const r = localStorage.getItem(LS_PLANNER);
    return r
      ? JSON.parse(r)
      : { globalPersons: 2, thresholds: { quick: 20, normal: 35 } };
  } catch {
    return { globalPersons: 2, thresholds: { quick: 20, normal: 35 } };
  }
};
          ...
          const loaded: PlannerSettings = {
            globalPersons:
              typeof data.defaultServings === "number"
                ...
                : planner.globalPersons,
            thresholds: {
              quick:
                typeof data.quickMealMinutes === "number"
                  ? data.quickMealMinutes
                  : planner.thresholds.quick,
              normal:
                typeof data.normalMealMinutes === "number"
                  ? data.normalMealMinutes
                  : planner.thresholds.normal,
            },
          };
          setPlanner(loaded);
          try {
            localStorage.setItem(LS_PLANNER, JSON.stringify(loaded));
          } catch {
            /* ignore */
          }
        }
  ...
  const updatePlanner = useCallback(
    (updated: PlannerSettings) => {
      ...
      saveRef.current = setTimeout(() => {
        try {
          localStorage.setItem(LS_PLANNER, JSON.stringify(updated));
        } catch {
          /* ignore */
        }
      ...
      <div style={{ padding: "0 12px", maxWidth: 600, margin: "0 auto" }}>
        <div style={{ display: "grid", gap: 12 }}>
          ...
          <section style={card}>
            ...
            <p style={sectionSub}>
              {t(
                "youllNeedYourCurrentPasswordToConfirm",
          ...
          {/* Planner settings */}
          <section style={card}>
            <h2 style={sectionTitle}>{t("mealPlanner", "Meal Planner")}</h2>
            <p style={sectionSub}>
              {t(
                "recipeManagementWeeklyPlanSettings",
                "Recipe management & weekly plan settings.",
              )}
            </p>
            ...
            >
              <div>
                <div style={{ fontSize: 14, fontWeight: 600 }}>
                  {t("defaultServings", "Default servings")}
                </div>
                <div
                  style={{
                    fontSize: 12,
                ...
                >
                  {t("appliesToNewPlanSlots", "Applies to new plan slots.")}
                </div>
              </div>
              <Stepper
                value={planner.globalPersons}
                onChange={(v) =>
                  updatePlanner({ ...planner, globalPersons: v })
                }
                min={1}
                max={20}
              />
            ...
            >
              {t("dayTypeLimitsMinutes", "Day type limits (minutes)")}
            </div>

            {[
              {
                emoji: "⚡",
                label: t("quickDay", "Quick day"),
                sub: t("maxTimeForFastMeals", "Max. time for fast meals"),
                val: planner.thresholds.quick,
                set: (v: number) =>
                  updatePlanner({
                    ...planner,
                    thresholds: {
                      ...planner.thresholds,
                      quick: Math.min(v, planner.thresholds.normal - 5),
                    },
                  }),
                min: 5,
                max: 60,
              ...
              {
                emoji: "🏠",
                label: t("normalDay", "Normal day"),
                sub: t(
                  "maxTimeForStandardMeals",
                  "Max. time for standard meals",
                ),
                val: planner.thresholds.normal,
                set: (v: number) =>
                  updatePlanner({
                    ...planner,
                    thresholds: {
                      ...planner.thresholds,
                      normal: Math.max(v, planner.thresholds.quick + 5),
                    },
                  }),
                min: 10,
                max: 120,
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <span style={{ fontSize: 18 }}>{t("key", "🌿")}</span>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>
                    {t("relaxed", "Relaxed")}
                  </div>
                  <div style={{ fontSize: 11, color: "var(--text-muted)" }}>
                    {t("allRecipesNoLimit", "All recipes, no limit")}
                  </div>
                </div>
              </div>
              <span
