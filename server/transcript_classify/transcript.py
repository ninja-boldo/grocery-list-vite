...
def ensure_wav(input_path: str, temp_dir: Path | None = None) -> str:
    ...
    if input_path.lower().endswith(".wav"):
        return input_path
    if (
        input_path.lower().endswith(".webm")
        or input_path.lower().endswith(".mp3")
        or input_path.lower().endswith(".m4a")
        or input_path.lower().endswith(".ogg")
    ):
        if temp_dir is None:
    ...
    raise Exception(
        "Only .wav, .webm, .mp3, .m4a, and .ogg files are supported at the moment"
    )
...
# -------------------------------
def transcribe(
    file_path: str, language="de", beam_size=1, task="transcribe", cloud=True
) -> str:
...
def main(argv):
    if len(argv) < 2:
        print(
            "Usage: python transcribe.py /path/to/audio.webm [--model small] [--device auto|cpu|cuda|mps] [--lang de] [--beam 1]"
        )
        return 1
