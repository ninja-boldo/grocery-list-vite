...
if os.environ.get("RUNNING_IN_CONTAINER") == "true":
    DEFAULT_CLASSIFIER_SERVER = "http://ollama:11434"

MODEL_NAME = "class-mapper"
...
class GroceryClassifier:
    def __init__(self, server_url: str = DEFAULT_CLASSIFIER_SERVER):
        ...
        print(f"[bold green]Classifier → ollama server:[/bold green] {server_url}")
    ...
    def classifyBatch(
        self, items: list[str], categories_list: list[list[str]] | None = None
    ) -> list[str]:
        """
        Batch classify.
