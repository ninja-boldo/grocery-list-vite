...
ON "users" ("user_id", "username", "password_hash");""",
...
ON "inventory" ("user_id", "is_wish", "created_at", "item_id", "id");""",
    ...
    """CREATE TABLE IF NOT EXISTS "wish_mapping" (
	"item_id" TEXT NOT NULL UNIQUE,
	"wish" TEXT DEFAULT NULL,
	PRIMARY KEY("item_id")
);""",
    """CREATE TABLE IF NOT EXISTS "grocery_offers" (
	"offer_id" INTEGER NOT NULL UNIQUE GENERATED BY DEFAULT AS IDENTITY,
    ...
    """CREATE INDEX "grocery_offers_index_0"
ON "grocery_offers" ("added_at", "item_name", "original_price", "offer_price", "supermarket_id", "is_app_offer", "offer_id");""",
    """CREATE TABLE IF NOT EXISTS "supermarkets" (
	"supermarket_id" INTEGER NOT NULL UNIQUE GENERATED BY DEFAULT AS IDENTITY,
	"name" TEXT,
	...
	"address" TEXT,
	PRIMARY KEY("supermarket_id", "name", "longitude", "latitude")
);""",
    """CREATE INDEX "supermarkets_index_0"
ON "supermarkets" ("supermarket_id", "name", "longitude", "latitude", "chain", "opening_periods", "address");""",
...
ON UPDATE NO ACTION ON DELETE NO ACTION;""",
    """ALTER TABLE "item_classification"
ADD FOREIGN KEY("item_id") REFERENCES "items"("item_id")
ON UPDATE NO ACTION ON DELETE NO ACTION;""",
    """ALTER TABLE "wish_mapping"
ADD FOREIGN KEY("item_id") REFERENCES "items"("item_id")
ON UPDATE NO ACTION ON DELETE NO ACTION;""",
    """ALTER TABLE "grocery_offers"
ADD FOREIGN KEY("supermarket_id") REFERENCES "supermarkets"("supermarket_id")
ON UPDATE NO ACTION ON DELETE NO ACTION;""",
]
        ...
        for item in body.items:
            for date, count in zip(item.perish_dates, item.per_date_count):
                await addItemToInventory(
                    ...
                    request.app.state.pool,
                    username=username,  # was: int(item.userId)
                    ean=item.ean,
...
async def rescanImageUrlsDaemon(pool: asyncpg.Pool, waitingTime: float = 300.0):
    try:
        while True:
...
async def classificationDaemon(pool: asyncpg.Pool, waitingTime: float = 30000.0):
    while True:
        try:
            await asyncio.sleep(waitingTime)
